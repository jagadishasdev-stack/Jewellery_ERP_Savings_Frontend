    ...
    <Box sx={{ textAlign: "center", py: 4 }}>
      <Avatar
        sx={{
          ...
          mx: "auto",
          mb: 2.5,
          boxShadow: "0 10px 30px rgba(24,20,12,0.18)",
        }}
      >
        <CheckCircleOutlineIcon sx={{ color: "#fff", fontSize: 46 }} />
      </Avatar>
      <Typography
...
const OrderSummary = () => {
  ...
  const handleProceed = () => {
    ...
    if (payMode === "cash") {
      ...
      // Push a history entry for the Payment step (route stays /cart) so Back
      // returns to the summary. The location.key effect flips proceedOnline.
      navigate("/cart", { state: { ecomStep: "Payment" } });
    }
  };
  ...
  return (
    <>
      <Box sx={{ px: 2, pb: 20 }}>
        {" "}
        <Typography sx={sectionHeadingSx}>Address</Typography>
        <Paper
          elevation={0}
          sx={{
        ...
        >
          <Typography
            sx={{
              fontSize: 14,
              fontWeight: shipping_address ? 600 : 400,
              color: shipping_address ? textColor : "#999",
              fontStyle: shipping_address ? "normal" : "italic",
            }}
          >
