...
function SelectPlan() {
  ...
  const { storePlans, storeAssets, branches } = useContext(StoreContext);
  ...
  // ── Branch filter (frontend-only) ─────────────────────────────────────────
  // allPlans = every branch's plans (fetched once with store_id only).
  // Defaults to the current branch so the first view is identical to before;
  // the dropdown lets the user switch (city shown, branch code as value).
  const [allPlans, setAllPlans] = useState(null);
  const [selectedBranch, setSelectedBranch] = useState(APP_CONFIG.BRANCH);

  useEffect(() => {
    const fetchAllBranchPlans = async () => {
      const url = `${process.env.REACT_APP_API_BASE_URL}/api/core/getGroups?store_id=${APP_CONFIG.STORE_ID}`;
      try {
        const response = await axios.get(url);
        setAllPlans(Array.isArray(response?.data) ? response.data : []);
      } catch (error) {
        // Non-fatal: the page keeps showing the current branch's plans from
        // StoreContext (existing behaviour) and the dropdown stays hidden.
        console.error("Failed to fetch all-branch plans:", error);
      }
    };
    fetchAllBranchPlans();
  }, []);

  // Until the all-branch fetch resolves, fall back to the context plans
  // (current branch) so the first paint is unchanged.
  const sourcePlans = allPlans ?? existingPlansList;

  // tbl_scheme_groups has no branch column at all — a group applies
  // tenant-wide, not per-branch — so /api/core/getGroups never returns a
  // `branch` field on any plan. Filtering on `p.branch === selectedBranch`
  // therefore matched nothing and silently emptied the list for every
  // tenant. Only actually filter when a plan carries real branch data;
  // otherwise (today, always) show it regardless of the selected branch.
  const branchFilteredPlans = useMemo(
    () =>
      selectedBranch === "ALL"
        ? sourcePlans
        : sourcePlans.filter((p) => !p.branch || p.branch === selectedBranch),
    [sourcePlans, selectedBranch],
  );
  ...
  //Pagination (over the branch-filtered list)
  const itemsPerPage = 10;
  const totalPages = Math.ceil(branchFilteredPlans?.length / itemsPerPage);
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = branchFilteredPlans?.slice(
    indexOfFirstItem,
    indexOfLastItem,
  );
  ...
  return (
    <>
      ...
      {/* Branch selector — every branch from the branch table (city shown,
          branch code as value); plans are filtered on the frontend */}
      {fallbackOpen === false && allPlans && (branches?.length || 0) > 1 && (
        <Box sx={{ width: "100%", mb: 1.5 }}>
          <BranchDropdown
            ...
            onChange={(code) => {
              ...
              setCurrentPage(1); // restart pagination for the new branch
            }}
          />
        </Box>
      )}

      {/* Empty state — selected branch (or store) has no plans */}
      {fallbackOpen === false &&
        branchFilteredPlans &&
        branchFilteredPlans.length === 0 && (
          <Box
            sx={{
              width: "100%",
              textAlign: "center",
          ...
          >
            <Typography
              sx={{ fontSize: 14, color: "#6b7280", fontWeight: 600, mb: 0.5 }}
            >
              No plans available in this store
            </Typography>
            <Typography sx={{ fontSize: 12, color: "#9ca3af" }}>
              Try another store to explore our saving plans
            </Typography>
            {selectedBranch !== "ALL" && (
              <Typography
                onClick={() => {
      ...
      {fallbackOpen === false &&
        ...
        branchFilteredPlans.length !== 0 && (
          ...
          >
            ...
            {branchFilteredPlans.length > itemsPerPage && (
