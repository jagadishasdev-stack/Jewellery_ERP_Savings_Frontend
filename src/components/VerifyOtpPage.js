...
const VerifyOtpPage = () => {
  ...
  const handleVerify = async () => {
    ...
    // Jewellery ERP's mobileAuth.js (see server/src/routes/mobileAuth.js)
    // handles send-otp/verify-otp for both flows — DLJ has no record in
    // the old savings-app MySQL database, so the old storeID-based
    // validateotp calls above literally can't resolve anything for it.
    setIsVerifying(true);
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_ERP_API_BASE_URL}/api/mobile/verify-otp`,
        {
          mobile,
          otp,
          tenantId: APP_CONFIG.TENANT_ID,
          purpose: type === "signup" ? "REGISTER" : "LOGIN",
          ...(type === "signup" && { signupData: state?.signupData }),
        },
      );
      ...
      await saveLoginData({
        ...
        adminUser: { ...data.user, token: data.token },
      });
      if (type === "login") await initPushNotifications();
