...
import { FONT_DISPLAY } from "./ui/ecomTokens";
...
// shipping_address may be a plain string (older orders) or an address object
// (newer orders). Return a readable single-line string for either.
...
const EmptyOrders = ({ onRefresh }) => (
  ...
  >
    ...
    <Typography sx={{ fontFamily: FONT_DISPLAY, fontSize: 19, fontWeight: 700, letterSpacing: "-0.01em", color: textColor, mb: 1 }}>
      No Orders Yet
    </Typography>
    <Typography
              ...
              <Box key={item.id}>
                ...
                <Box sx={{ px: 2, py: 1.8 }}>
                  ...
                  <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                    ...
                    />
                    <Chip
                      label={`Purity: ${item.purity_name || item.purity}`}
                      size="small"
                      sx={{
                        fontSize: 11,
                        height: 22,
...
const MyOrders = () => {
  ...
  // Defensive: avoid a hard crash if this mounts before StoreContext has
  // finished loading storeAssets — same values once it's loaded.
  ...
  // Push the top toast below the fixed app header (safe-area + 56px header)
  // so it isn't hidden behind it.
  const safeAreaTop = useSafeAreaTop();
  const snackbarSx = { top: `calc(${safeAreaTop} + 64px) !important` };
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    try {
      const url = `${process.env.REACT_APP_API_BASE_URL}/api/e-com/orders/user?store_id=${store_id}&branch_id=${branch}&user_id=${adminUser?.user_id}&is_alpha=${APP_CONFIG.IS_ALPHA}`;
  ...
  const fetchOrderDetail = async (order) => {
    ...
    try {
      const url = `${process.env.REACT_APP_API_BASE_URL}/api/e-com/orders/user/${order.order_id}?store_id=${store_id}&branch_id=${branch}&user_id=${adminUser?.user_id}&is_alpha=${APP_CONFIG.IS_ALPHA}`;
      const response = await fetch(url, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const result = await response.json();
  ...
  return (
    <>
      ...
      >
        ...
        <Box sx={{ flex: 1 }}>
          <Typography sx={{ fontFamily: FONT_DISPLAY, fontSize: 18, fontWeight: 700, letterSpacing: "-0.01em", color: textColor }}>
            My Orders
          </Typography>
          {!loading && (
