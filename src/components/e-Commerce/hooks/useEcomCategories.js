  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        ...
        // Keep every named category — the consumer decides whether to render
        // image tiles (all images present) or name-only chips (any missing).
        setCategories(raw.filter((item) => item.name));
      } catch (err) {
        if (!cancelled) {
