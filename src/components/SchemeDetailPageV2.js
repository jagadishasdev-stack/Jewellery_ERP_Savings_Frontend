...
// Loads PhonePe's checkout.js once (used by the web IFRAME PayPage flow).
const loadPhonePeCheckoutScript = () =>
  new Promise((resolve) => {
    if (window.PhonePeCheckout) return resolve(true);
...
// Android major version from the WebView user-agent ("Android 14" → 14); 0 if
// unknown / not Android. Used to gate the iframe fallback to Android < 15.
const getAndroidMajorVersion = () => {
  const m = /Android\s+(\d+)/i.exec(
    (typeof navigator !== "undefined" && navigator.userAgent) || "",
  );
  return m ? parseInt(m[1], 10) : 0;
};

// Store-11 / store-5 customer who must always transact on PhonePe, whatever the branch
// has configured on the backend (see resolvePaymentGateway).
const FORCE_PHONEPE_STORE_IDS = [11, 5];
const FORCE_PHONEPE_MOBILE = "8147128889";
...
const SchemeDetailPageV2 = () => {
  ...
  // "iframe" only when the hidden 3-tap override on PaymentScreen was used;
  // otherwise undefined → the default native SDK flow runs (see runPhonePe).
  const paymentVersion = location?.state?.paymentVersion;

  const { adminUser, loginRole, loginMobile } = useContext(AuthContext);
  ...
  const resolvePaymentGateway = useCallback(async () => {
    ...
    // Store 11 / store 5 + this one customer always routes to PhonePe; every other
    // store/customer keeps whatever the backend returned.
    const forcePhonePe =
      FORCE_PHONEPE_STORE_IDS.includes(APP_CONFIG.STORE_ID) &&
      [loginMobile, adminUser?.mobile].some(
        (m) => String(m ?? "").trim() === FORCE_PHONEPE_MOBILE,
      );
    ...
    return {
      ...
      razorpay_std: forcePhonePe ? "52" : pgDetails.standered,
      razorpay_env: pgDetails.env_ment,
    };
  }, [gateway, useBranch, loginMobile, adminUser?.mobile]);
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // Web IFRAME PayPage flow (checkout.js). Used ONLY via the hidden override
  // for eligible devices (see the guard in runPhonePe). Mirrors the SDK flow's
  // create-order → verify → save sequence; the only differences are the
  // create-order-web endpoint (returns a redirectUrl instead of an order
  // token) and opening PhonePe's iframe instead of the native SDK. Reuses the
  // same verifyPhonePe/goToSuccess/goAfterCancel as the SDK flow.
  const runPhonePeIframe = async (pgateway, record, obj) => {
    try {
      const epayRefRes = await axios.get(
        `${API_BASE}/api/core/getPaymentGatewayReference`,
      );
      ...
      const createOrderRes = await axios.post(
        `${API_BASE}/api/phonepe/create-order-web`,
        ppay_obj,
      );
      const { redirectUrl, merchantOrderId } = createOrderRes.data || {};

      if (!redirectUrl || !merchantOrderId) {
        showAlert("Error creating order", "No redirect URL returned from server");
        setPageLoading(false);
        return;
      }
      ...
      const handleSuccess = async () => {
        ...
        setVerifying(true);
        await verifyPhonePe(merchantOrderId, savePhonePeSuccess, goToFailed);
      };

      const scriptLoaded = await loadPhonePeCheckoutScript();
      if (!scriptLoaded || !window.PhonePeCheckout) {
        showAlert(
          "Error",
          "Failed to load PhonePe checkout. Please check connection.",
        );
        setPageLoading(false);
        return;
      }
      ...
      console.error("Error during PhonePe iframe payment:", error);
  ...
  const runPhonePe = async (pgateway, record, obj) => {
    ...
    // Hidden iframe override: engage ONLY for an eligible device (Android < 15)
    // AND an explicit user action (paymentVersion === "iframe", non-agent).
    // Every other case falls through to the unchanged default native SDK flow.
    const androidMajor = getAndroidMajorVersion();
    const useIframe =
      paymentVersion === "iframe" &&
      Capacitor.getPlatform() === "android" &&
      androidMajor > 0 &&
      androidMajor < 15 &&
      loginRole !== "agent";
    if (useIframe) {
      await runPhonePeIframe(pgateway, record, obj);
      return;
    }
    ...
    try {
      ...
      // 🧪 SANDBOX only: read the real order state from PhonePe and route on it.
      // COMPLETED → success page.  PENDING / FAILED → payment-failed page.
      const checkSandboxStatus = async (result) => {
        try {
          const { data } = await axios.post(
            `${API_BASE}/api/phonepe/order-status`,
            {
              merchant_order_id,
              store_id: APP_CONFIG.STORE_ID,
              branch_id: useBranch,
          ...
          // PENDING / FAILED → same failed page the normal flow uses.
          navigate("/paymentfailed", {
            state: {
              reason: "CANCELLED",
              message:
          ...
          alert(`order-status failed: ${e.message}`);
        }
      };
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        try {
          ...
          if (pgateway?.razorpay_env === "SANDBOX" && result.status === "SUCCESS") {
            await checkSandboxStatus(result);
            return;
          }
...
// /**
//  * ✅ ROLLBACK SWITCH
//  * true  → PhonePe opens via checkout.js in an IFRAME (new, faster, no native
//  *         plugin dependency, works identically on Android/iOS/web).
//  * false → falls straight back to the old PhonePePaymentPlugin native SDK
//  *         flow, unchanged. Flip this if the new flow misbehaves on any
//  *         device in production — no redeploy of backend needed either way,
//  *         both endpoints stay live.
//  */
// const USE_PHONEPE_IFRAME = true;
...
//   // ══════════════════════════════════════════════════════════════════════════
//   // RAZORPAY  (merged iOS + Android + Web, razorpay_std === "0")
//   // Unchanged — not part of this PhonePe migration.
//   // ══════════════════════════════════════════════════════════════════════════
//   const runRazorpay = async (pgateway, record, obj) => {
//     try {
//       // 1️⃣ Unique payment-gateway reference number.
...
//   // ══════════════════════════════════════════════════════════════════════════
//   // PHONEPE  (razorpay_std === "52")
//   //
//   // ✅ NEW default: checkout.js loaded dynamically (same pattern as
//   //    loadRazorpayWebScript above) → window.PhonePeCheckout.transact({
//   //    tokenUrl, type: "IFRAME", callback }). Works identically on
//   //    Android, iOS and web — no native plugin bridge, no app-switch,
//   //    the PayPage opens right inside the existing screen.
//   //
//   // Legacy PhonePePaymentPlugin native-SDK flow is kept below, completely
//   // unchanged, and runs instead whenever USE_PHONEPE_IFRAME is set to false.
//   // ══════════════════════════════════════════════════════════════════════════
//   const loadPhonePeCheckoutScript = () =>
//     new Promise((resolve) => {
//       if (window.PhonePeCheckout) return resolve(true);
...
//       // ── Shared, used by BOTH the new iframe flow and the legacy SDK flow ──
...
//         setVerifying(true);
//         await verifyPhonePe(currentMerchantOrderId, savePhonePeSuccess, goToFailed);
//       };

//       // ══════════════════════════════════════════════════════════════════
//       // ✅ NEW: checkout.js IFRAME PayPage flow — default path
//       // ══════════════════════════════════════════════════════════════════
//       if (USE_PHONEPE_IFRAME) {
//         const createOrderRes = await axios.post(
//           `${API_BASE}/api/phonepe/create-order-web`,
//           ppay_obj,
//         );
...
//             "No redirect URL returned from server",
//           );
//           setPageLoading(false);
//           return;
//         }
...
//       // ══════════════════════════════════════════════════════════════════
//       // Legacy native SDK flow — unchanged. Flip USE_PHONEPE_IFRAME to
//       // false above to fall back to this instantly if needed.
//       // ══════════════════════════════════════════════════════════════════
//       const createOrderRes = await axios.post(
//         `${API_BASE}/api/phonepe/create-order`,
//         ppay_obj,
...
//       const order_ID = createOrderRes.data?.orderId;
//       const merchant_order_id = createOrderRes.data?.merchantOrderId;
//       currentMerchantOrderId = merchant_order_id;
...
//       // Native PhonePe transaction (Android/iOS).
//       if (Capacitor.isNativePlatform()) {
//         if (isCancelled.current) return;
//         try {
//           if (!phonePeReady) {
...
//   // PhonePe verification loop: poll the DB, fall back to manual checks,
//   // up to a 5-minute ceiling (unchanged backend contract & timings).
//   // Used identically by both the iframe flow and the legacy SDK flow.
//   const verifyPhonePe = async (merchantOrderId, onSuccess, onFailed) => {
//     const MAX_DURATION = 300000; // 5 minutes
//     const CHECK_INTERVAL = 5000;
//     const ROUND_DURATION = 15000;
